...
#define LEFTBORDER 130
#define TOPBORDER 380
#define INTERVAL 120
    ...
    : QMainWindow(parent)
{
    ...
    PvPStart->setGeometry(340,1460,400,110);
    PvPStart->setText("新的PVP对局");
    PvEStart=new QPushButton(this);
    PvEStart->setGeometry(340,1580,400,110);
    PvEStart->setText("新的人机对局");
    Help=new QPushButton(this);
    Help->setGeometry(340,1700,400,110);
    Help->setText("查看游戏帮助");
    for(int i=0;i<7;i++)
        for(int j=0;j<7;j++)
        {
...
void MainWindow::GameCore()
{
    ...
    if(!BlackAva)
    {
        ...
        GameInfo2->setText("Winner Winner! Chicken dinner!");
    ...
    if(Color)
    {
        ...
        else if(msg[0]==8)
        {
            ...
            else
            {
                ...
                GameInfo2->setText("Good luck next time");
            }
            for(i=0;i<7;i++)
                for(j=0;j<7;j++)
                    if(GridInfo[i][j]==1)
